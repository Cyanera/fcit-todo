...
test('التعميم الرسمي: العنوان من فقرة الطلب لا من التحية', () => {
  // تعميم حقيقي وصل قروب العمل — كان العنوان يبدأ بـ«السلام عليكم»
  const circular = `السلام عليكم ورحمة الله وبركاته،
...
وفي حال وجود أي تعثر أو ملفات ناقصة لدى أحد أعضاء هيئة التدريس، نأمل منكم العمل على معالجتها.

مع خالص الشكر والتقدير لكم جميعًا.`;

  const task = run(circular);
  assert.ok(task, 'التعميم مهمة');
  assert.equal(task.title, 'الانتهاء من استكمال ملفات المقررات');
  assert.ok(!task.title.includes('السلام'), 'التحية لا تدخل العنوان');
  assert.ok(!task.title.includes('*'), 'تنسيق واتساب العريض يُزال');
  assert.ok(task.due_date, 'الموعد يُستخرج رغم تركيبه');
});
